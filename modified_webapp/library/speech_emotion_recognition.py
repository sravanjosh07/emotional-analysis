...
class speechEmotionRecognition:
    ...
    def voice_recording(self, filename, duration=5, sample_rate=16000, chunk=1024, channels=1):
        ...
        global audio_start_stop #Defined a global variable inorder to start and stop the recording
        ...
        while True:
            ...
            # amp = sns.stripplot(amplitude)
            # # sns_plot = sns.pairplot(df, hue='species', height=2.5)
            # # sns_plot.savefig("output.png")
            # amp.savefig('tmp/j.jpg', frame)

            # Yield the image at each step
            yield (b'--frame\r\n'
            b'Content-Type: image/jpeg\r\n\r\n' + open('tmp/google.jpeg.webp', 'rb').read() + b'\r\n')
            if audio_start_stop == 'stop':
                print('* End Recording * ')
                stream.stop_stream()
                stream.close()
                p.terminate()

...
def gen():
    ...
    # To run it infinitely until we stop
    while True:
        k = k+1
        end = time.time()
        ...
        for (i, rect) in enumerate(rects):
            ...
            angry_0_total.append(prediction[0][0].astype(float))
            disgust_1_total.append(prediction[0][1].astype(float))
            fear_2_total.append(prediction[0][2].astype(float))
            happy_3_total.append(prediction[0][3].astype(float))
            sad_4_total.append(prediction[0][4].astype(float))
            surprise_5_total.append(prediction[0][5].astype(float))
            neutral_6_total.append(prediction[0][6].astype(float))
            ...
            predictions_total.append(str(prediction_result))
        ...
        # Once reaching the time interval, write the results to the personal file and to the overall file
        if end-start > time_interval :
            rows = zip(angry_0,disgust_1,fear_2,happy_3,sad_4,surprise_5,neutral_6)
            with open("static/js/db/histo_perso_check.txt", "w") as d:
                d.write("density"+'\n')
                for val in predictions :
                    d.write(str(val)+'\n')

            with open("static/js/db/prob_check.csv", "w") as d:
                writer = csv.writer(d)
                for row in rows:
                    writer.writerow(row)


            # with open("static/js/db/prob_tot_check.csv", "a") as d:
            #     writer = csv.writer(d)
            #     for row in rows:
            #         writer.writerow(row)
        ...
        with open("static/js/db/histo_check.txt", "w") as d:
            for val in predictions_total :
                d.write(str(val)+'\n')
        rows_total = zip(angry_0_total,disgust_1_total,fear_2_total,happy_3_total,sad_4_total,surprise_5_total,neutral_6_total)
        with open("static/js/db/prob_tot_check.csv", "w") as d:
            writer = csv.writer(d)
            for row in rows_total:
                writer.writerow(row)
        print("Recording")
        if video_start_stop == 'stop':

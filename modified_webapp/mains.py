...
import tempfile
import library.video_emotion_recognition as vi
import library.speech_emotion_recognition as sp
...
@app.route('/video_show', methods=['POST'])
def video_idk():
    return render_template('video_show.html')


# Dashboard
@app.route('/video_dash', methods=['POST'])
def video_dash():
    vi.video_start_stop = 'stop'
    # Load personal history
    df_2 = pd.read_csv('static/js/db/histo_check.txt', header = None)
    ...
    ### Altair Plot
    df_altair = pd.read_csv('static/js/db/prob_tot_check.csv', header=None, index_col=None).reset_index()
    ...
    # os.remove('static/js/db/histo_check.txt')
    # os.remove('static/js/db/prob_tot_check.csv')
...
#########################################################################
@app.route('/video_temp', methods=['GET'])
def video_temp():
    # Load personal history
    df_2 = pd.read_csv('static/js/db/histo_check.txt', header = None)
    df_2.columns = ['density']
    ...
    return jsonify( emo=emotion_label(emotion),  prob = emo_prop(df_2))
...
# Audio Recording
@app.route('/audio_recording', methods=["GET"]) #changed the method to GET only
def audio_recording():
    ...
    rec_sub_dir = os.path.join('tmp','voice_recording_1.wav')
    return Response(SER.voice_recording(rec_sub_dir, duration=rec_duration),mimetype='multipart/x-mixed-replace; boundary=frame')
...
def audio_show():
    return render_template('audio_show.html')


# Audio Emotion Analysis
@app.route('/audio_dash', methods=("POST", "GET"))
def audio_dash():
    sp.audio_start_stop = 'stop'
...
#####################################################################################
@app.route('/audio_temp', methods=["GET"])
def audio_temp():
    ...
    #return the value
    return jsonify(emo=major_emotion,  prob=emotion_dist)
